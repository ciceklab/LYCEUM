'''
LYCEUM samples preprocessing source code for training.
...
required_args.add_argument("-o", "--output", help="Please provide the output path.\n This path will contain .npy files for each exons in every samples.", required=True)
...
DELETION_FOLDER = os.path.join(ROOT_FOLDER,"deletion")
DUPLICATION_FOLDER = os.path.join(ROOT_FOLDER,"duplication")
NOCALL_FOLDER = os.path.join(ROOT_FOLDER,"nocall_0")
...
ctr=0
for i, item in tqdm(enumerate(all_samples_names)):
    if i%25 == 0:
        NOCALL_FOLDER = os.path.join(ROOT_FOLDER, NOCALL_FOLDER.split("_")[-2].split("/")[-1]+f"_{ctr}")
        os.makedirs(NOCALL_FOLDER, exist_ok=True)
        ctr += 1
