...
This script generates and processes the samples to finet-tune the model.
'''
import numpy as np
import os
...
required_args.add_argument("-gt", "--groundtruth", help="Please provide the path of groundtruth calls.", required=True)
...
for file_name in tqdm(exon_wise_readdepth_files):
    ...
    wgs_calls_data = pd.read_csv(os.path.join(args.groundtruth,f"{sample_name.split('_')[0]}.csv") , sep=",",comment='#',header=None).values 
